...
size_t ServerHello::serialize(std::basic_ostringstream<uint8_t>& sstr)
{
    size_t len = 0;
    size_t ret = 0;
    ...
    sstr.write(this->random.data(), this->random.siza());
...
void ServerHello::deserialize(std::basic_istringstream<uint8_t>& sstr) 
{
    int len = 0;
